...
export class RgbColor extends AbstractColor<RgbColor, RgbComponent> {
	...
	fancyCss(): string {
		const [r, g, b] = this.values().map((c) => Math.floor(c * 255));
		return `rgb(<span class="dark:text-red-300 text-red-500">${r}</span>, <span class="dark:text-green-300 text-green-500">${g}</span>, <span class="dark:text-blue-300 text-blue-500">${b}</span>)`;
	}
	...
	fancyHex(): string {
		...
		const str = [r, g, b]
			.map((comp, index) => {
				...
				return `<span class="dark:text-${color}-300 text-${color}-500">${str}</span>`;
			})
			.reduce((s1, s2) => s1 + s2);
		return '#' + str;
	}
