...
<Container>
	...
	{#if currentStep == 1}
		<Gallery class="gap-4 md:grid-cols-4 grid-cols-2 mx-4 mt-4">
			<Card transition={scale} size="lg" class="col-span-2">
				...
				<Gallery class="gap-6 grid-cols-2">
					...
					<Button class="mt-2 text-xl" on:click={addNumber}>Hinzufügen</Button>
				</Gallery>
			</Card>
			{#each numbers as number, index (number.id)}
				<div animate:flip={{ duration: 200, delay: index * 50 }}>
					...
					>
						<h5
							class="mb-2 text-8xl font-light mx-auto tracking-tight text-gray-900 dark:text-white"
						>
							{number.value}
						</h5>
					</Card>
